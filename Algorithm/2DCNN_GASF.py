...
class TwoDCNN(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.fc1 = nn.Linear(32 * (100 // 4) * (100 // 4), 64)
        self.relu3 = nn.ReLU()
...
# 读取数据
df = pd.read_csv('an_data.csv', encoding='ISO-8859-1')  # 你可以根据情况选择适当的编码格式

# 提取标签和特征
y = df['Name'].values
# y = df['RefTP'].values
X = df['AN_Spec'].apply(lambda x: np.array(list(map(float, x.split())))[350:450]).values
X = np.vstack(X)
...
joblib.dump(scaler, 'scaler_2dcnn.pkl')  # 保存标准化器到文件

# 将一维数据转换为 GASF
X_gasf = np.array([gasf(x) for x in X])
X_gasf = X_gasf[:, np.newaxis, :, :]  # 添加通道维度

X_train, X_temp, y_train, y_temp = train_test_split(X_gasf, y, test_size=0.3)  # 70%训练，30%临时
...
# df_data.to_csv('training_metrics_2dcnn_gasf_tp2.csv', index=False)
...
# torch.save(model.state_dict(), 'model_2dcnn_gasf_tp2.pth')  # 保存模型的权重和参数