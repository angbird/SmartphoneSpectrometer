...
# 4. 建立 PLS 模型
pls = PLSRegression(n_components=15)  # 可调 n_components
pls.fit(target_cal, standard_cal)

# 5. 对测试集进行预测校正
corrected_test = pls.predict(target_test)

# 6. 保存校正结果（可选）
# np.savetxt('pls_corrected_test.csv', corrected_test, delimiter=',')
